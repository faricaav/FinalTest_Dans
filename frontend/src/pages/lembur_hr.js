...
export default function LemburHr(){
    ...
    const getLembur = () => {
        axios.get("http://localhost:8080/lembur/", {
          headers: {
    ...
    const handleApprove = (item) => {
        ...
        if(window.confirm('are you sure to approve?')){
          axios.put("http://localhost:8080/lembur/approve/" + item.id_lembur, form, {
            headers: {
              Authorization: `Bearer ${token}`,
            }})
            .then(response => {
    ...
    const handleReject = (item) => {
      ...
      if(window.confirm('are you sure to reject?')){
        axios.put("http://localhost:8080/lembur/reject/" + item.id_lembur, form, {
          headers: {
            Authorization: `Bearer ${token}`,
          }})
          .then(response => {
