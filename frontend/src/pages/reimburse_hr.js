...
export default function ReimburseHr(){
    ...
    const getReimburse = () => {
        axios.get("http://localhost:8080/reimburse/", {
          headers: {
    ...
    const handleApprove = (item) => {
        ...
        if(window.confirm('are you sure to approve?')){
          axios.put("http://localhost:8080/reimburse/approve/" + item.id_reimburse, form, {
            headers: {
              Authorization: `Bearer ${token}`,
            }})
            .then(response => {
    ...
    const handleReject = (item) => {
      ...
      if(window.confirm('are you sure to reject?')){
        axios.put("http://localhost:8080/reimburse/reject/" + item.id_reimburse, form, {
          headers: {
            Authorization: `Bearer ${token}`,
          }})
          .then(response => {
